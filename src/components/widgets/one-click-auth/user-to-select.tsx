...
const UserToSelect = ({ onSelect, isCurrent = false, isSelected = false, uid, noBorder = false }: Props) => {
  ...
  useEffect(() => {
    userAPI.byId.short(uid).then((result) => setUser(result))
  }, [uid])
  if (!user) return null
  return (
    <div
      onClick={() => user && onSelect && onSelect(user)}
      className={cn(
    ...
    >
      <div className="w-10 aspect-square relative flex items-center justify-center">
        {isCurrent && (
          <div className="absolute z-10 w-full h-full flex items-center justify-center rounded-full bg-green-600">
